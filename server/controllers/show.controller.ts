...
const updateShowHandler = async (req: Request<ShowUpdateType['params']>, res: Response) => {
  try {
    const user = res.locals.user._id;
...
const getShowHandler = async (req: Request<ShowGetType['params']>, res: Response) => {
  try {
    const user = res.locals.user._id;
    const id = req.params.id;
...
const deleteShowHandler = async (req: Request<ShowDeleteType['params']>, res: Response) => {
  try {
    const user = res.locals.user._id;
    const id = req.params.id;
    const success = await deleteShow({ user, id });
    if (!success) return res.status(404).send('Show not found.');
