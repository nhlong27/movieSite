...
const SignInContainer = ({
  ...
  const navigate = (path: string | 0) => {
    path === 0 ? useRouter().reload() : useRouter().push(path)
  }
  ...
  return (
    <div className="w-full h-full p-4 flex flex-col justify-start items-center font-poppins bg-transparent">
      ...
      <FormComponent
        ...
        options={[
          {
            extras: { type: 'text', placeholder: 'Your_name@gmail.com' },
            name: 'email',
          },
          {
            extras: { type: 'password', placeholder: 'My_password' },
            name: 'password',
          },
        ]}
      />
